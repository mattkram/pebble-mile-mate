...
static void prv_draw_cell_backgrounds(Layer *layer, GContext *ctx) {
  ...
  for (int i = 0, current_x_offset = 0; i < data->num_cells; i++) {
    ...
    // Add extra space for decimal point
    if (data->num_decimals > 0 && i == data->num_cells - data->num_decimals - 1) {
      current_x_offset += DECIMAL_CELL_WIDTH;
    }
...
static void prv_draw_text(Layer *layer, GContext *ctx) {
  ...
  for (int i = 0, current_x_offset = 0; i < data->num_cells; i++) {
    ...
    if (data->num_decimals > 0 && i == data->num_cells - data->num_decimals - 1) {
      ...
      current_x_offset -= data->cell_padding / 2;
      ...
      GRect rect = GRect(current_x_offset, y_offset, DECIMAL_CELL_WIDTH, height);
      graphics_draw_text(ctx, ".", data->font, rect, GTextOverflowModeFill, GTextAlignmentCenter, NULL);
      current_x_offset += DECIMAL_CELL_WIDTH + data->cell_padding / 2;
...
static Layer* selection_layer_init(SelectionLayerData *selection_layer_, GRect frame, int num_cells, int num_decimals) {
...
Layer* selection_layer_create(GRect frame, int num_cells, int num_decimals) {
  SelectionLayerData *selection_layer_data = NULL;
  return selection_layer_init(selection_layer_data, frame, num_cells, num_decimals);
}
